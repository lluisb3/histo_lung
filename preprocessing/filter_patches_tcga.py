...
def filter_patches(list_dirs, maskdir):
    ...
    for filename in tqdm(list_dirs, desc="Filtering patches from PyHIST"):
        ...
        binary_mask = cv.imread(str(Path(maskdir / filename.parent.stem / f"{filename.stem}"
                f"{filename.suffixes[0]}" / f"binary_{filename.stem}{filename.suffixes[0]}.png")))
        ...
        slide = openslide.OpenSlide(str(Path(datadir / filename.parent.stem /f"{filename.stem}{filename.suffixes[0]}.svs" )))
        ...
        outputdir_metadata = Path(filename / f"{filename.stem}{filename.suffixes[0]}_densely_filtered_metadata.csv")
        ...
        outputdir_paths = Path(filename / f"{filename.stem}{filename.suffixes[0]}_densely_filtered_paths.csv")
        ...
        print(f"Filtered .csv for {filename.stem}{filename.suffixes[0]} saved on {outputdir_paths.parent}")
