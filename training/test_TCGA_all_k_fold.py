...
def main(experiment_name):
      ...
      for fold in subdirs:
            ...
            bestdir = Path(modeldir / fold / cfg.dataset.magnification / cfg.model.model_name)

            outputdir_fold = Path(testdir / "test_metrics_all" / experiment_name / fold)
            Path(outputdir_fold).mkdir(exist_ok=True, parents=True)
            ...
            with torch.no_grad():
                  ...
                  filename_test_predictions = Path(outputdir_fold / f"test_predictions_{epoch + 1}.csv")
                  df_predictions.to_csv(filename_test_predictions)

                  y_pred = np.append(y_pred, [1,1,1,1])
                  y_true = np.append(y_true, [1,1,1,1])
                  scores_pred= np.append(scores_pred, [1,1,1,1])

                  arange_like_predictions = np.arange(len(y_true))
                  
                  y_pred_reshape = y_pred.reshape(len(test_dataset)+1, 4)
                  y_true_reshape = y_true.reshape(len(test_dataset)+1, 4)
                  scores_pred_reshape = scores_pred.reshape(len(test_dataset)+1, 4)
                  ...
                  plt.savefig(outputdir_fold / f"test_{epoch + 1}_cm_{names[0]}.png")
                  plt.close()
                  ...
                  plt.savefig(outputdir_fold / f"test_{epoch + 1}_cm_{names[1]}.png")
                  plt.close()
                  ...
                  plt.savefig(outputdir_fold / f"test_{epoch + 1}_cm_{names[2]}.png")
                  plt.close()
                  ...
                  plt.savefig(outputdir_fold / f"test_{epoch + 1}_cm_{names[3]}.png")
                  plt.close()
                  ...
                  plt.savefig(outputdir_fold / f"test_{epoch + 1}_roc.png")
                  plt.close()
                  ...
                  plt.savefig(outputdir_fold / f"test_{epoch + 1}_pr_curve.png")
                  plt.close()
