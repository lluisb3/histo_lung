...
models_path = str(Path(thispath.parent / "models_train" ))
...
model_weights_temporary_filename = models_path+'MoCo_temporary.pt'
...
pipeline_transform = A.Compose([
	...
	#A.RandomContrast(limit=0.2, p=prob),
	#A.GaussNoise(p=prob),
	A.ElasticTransform(alpha=200, sigma=10, alpha_affine=10, interpolation=2, border_mode=4, p=prob),
	A.GridDistortion(num_steps=1, distort_limit=0.2, interpolation=1, border_mode=4, p=prob),
	A.GlassBlur(sigma=0.1, max_delta=1, iterations=1, p=prob),
	A.OpticalDistortion (distort_limit=0.2, shift_limit=0.2, interpolation=1, border_mode=4, value=None, p=prob),
	#A.GridDropout (ratio=0.3, unit_size_min=3, unit_size_max=40, holes_number_x=3, holes_number_y=3, shift_x=1, shift_y=10, random_offset=True, fill_value=0, p=prob),
	A.Equalize(p=prob),
	#A.Posterize(p=prob, always_apply=True),
...
pipeline_transform_soft = A.Compose([
	...
	A.HueSaturationValue(hue_shift_limit=(-10,10),sat_shift_limit=(-10,10),val_shift_limit=(-5,5),p=p_soft),
...
for idx_wsi, wsi in enumerate(train_dataset):
